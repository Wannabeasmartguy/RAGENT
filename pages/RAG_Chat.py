...
if "rag_chat_config_list" not in st.session_state:
    st.session_state.rag_chat_config_list = [chat_history_storage.get_specific_run(st.session_state.rag_run_id).llm]
...
with st.sidebar:
    ...
    with rag_dialog_settings_tab:
        ...
        def rag_saved_dialog_change_callback():
            ...
            st.session_state.rag_chat_config_list = [chat_history_storage.get_specific_run(st.session_state.rag_saved_dialog.run_id).llm]
            try:
                st.session_state.rag_chat_history = chat_history_storage.get_specific_run(st.session_state.rag_saved_dialog.run_id).memory["chat_history"]
            except:
        ...
        with delete_dialog_column:
            def delete_rag_dialog_callback():
                ...
                if len(chat_history_storage.get_all_run_ids()) == 0:
                    ...
                    st.session_state.rag_run_id = chat_history_storage.get_all_run_ids()[0]
                    st.session_state.rag_chat_config_list = [chat_history_storage.get_specific_run(st.session_state.rag_run_id).llm]
                    st.session_state.custom_rag_chat_history = chat_history_storage.get_specific_run(st.session_state.rag_run_id).memory["chat_history"]
                    st.session_state.custom_rag_sources = chat_history_storage.get_specific_run(st.session_state.rag_run_id).task_data["source_documents"]
    ...
    with rag_model_settings_tab:
        ...
        def get_model_type_index():
            ...
            try:
                return options.index(chat_history_storage.get_specific_run(st.session_state.rag_run_id).assistant_data["model_type"])
            except:
                return 0
        ...
        with st.expander(label=i18n("Model config"),expanded=True):
            max_tokens = st.number_input(
                ...
                min_value=1,
                value=config_list_postprocess(st.session_state.rag_chat_config_list)[0].get("max_tokens", 1900),
                step=1,
                key="max_tokens",
                on_change=update_rag_config_in_db_callback,
            ...
            temperature = st.slider(
                ...
                min_value=0.0,
                max_value=2.0,
                value=config_list_postprocess(st.session_state.rag_chat_config_list)[0].get("temperature", 0.5),
                step=0.1,
                key="temperature",
                on_change=update_rag_config_in_db_callback,
            ...
            top_p = st.slider(
                ...
                min_value=0.0,
                max_value=1.0,
                value=config_list_postprocess(st.session_state.rag_chat_config_list)[0].get("top_p", 0.5),
                step=0.1,
                key="top_p",
                on_change=update_rag_config_in_db_callback,
            ...
            if_stream = st.toggle(
                label=i18n("Stream"),
                value=config_list_postprocess(st.session_state.rag_chat_config_list)[0].get("stream", True),
                key="if_stream",
                on_change=update_rag_config_in_db_callback,
        ...
        if select_box0 != "Llamafile":
            def get_selected_non_llamafile_model_index(model_type) -> int:
                try:
                    return model_selector(model_type).index(st.session_state.rag_chat_config_list[0].get("model"))
                except:
                    return None
